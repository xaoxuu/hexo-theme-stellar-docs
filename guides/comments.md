...
设置 `provider: null` 会全站停用评论。`title` 可设置评论区标题，空字符串会隐藏标题。从 rc.3 起可在服务对象覆盖客户端资源：六个 Provider 均支持 js，Waline 和 Artalk 支持 css，Waline 另支持 meta_css；页面在 comments.options 中覆盖。省略或 null 使用默认资源；Artalk 默认从最终 server 的 dist 目录加载配套 JS/CSS。src 与 inject 不是资源覆盖入口。rc.5 起发现评论容器后立即异步加载，不等待进入视口，也不阻塞其它页面功能初始化。
