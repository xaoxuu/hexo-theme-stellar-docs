...
| 字段 | 输出位置 | 常见用途 |
| :--- | :--- | :--- |
| `head_begin` | `<head>` 后、主题 meta 前 | 必须早于主题 Head 内容的声明 |
| `head_end` | `</head>` 前 | 样式表、meta 与 Head 脚本 |
| `body_begin` | `<body>` 后、页面外壳前 | 必须先于页面外壳的结构或脚本 |
| `body_end` | `</body>` 前 | 统计与延后加载脚本 |

四个字段都只接受字符串，并会原样插入 HTML，不进行解析、格式化或转义，因此只应填写维护者完全信任的内容。页面 Front Matter 也可用同名字段追加本页资源；同一位置总是先输出站点配置，再输出页面内容，中间插入一个换行。Hexo 根 `_config.yml` 中的 `inject` 不会被主题读取。
...
统计等第三方脚本可通过 `inject.body_end` 接入，参数、数据和隐私设置需按服务方说明配置。只有确实要求早于页面内容运行的代码才使用 `head_begin` 或 `body_begin`。主题自己的脚本由主题加载，旧版资源字段不再用于替换这些脚本。
