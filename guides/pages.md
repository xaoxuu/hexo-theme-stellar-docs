...
tagline: 写给周末的一段小记
banner:
  image: /images/walk-banner.webp
  headline: 一次散步
  tagline: 午后，沿溪向北
```

根级 `cover/tagline` 用于列表卡片，`banner` 用于内容页顶部。没有图片时可省略；Collection 的封面不会自动成为成员文章的封面。
...
footer:
  ...
  license: false
  share: [wechat, link, system]
comments:
  enabled: false
```

菜单、三个 Region、页脚和评论都可按页覆盖。普通 Post 默认使用全局 Article 页脚；Wiki、Topic、Notebook Collection 默认关闭分享，可在 Collection 或页面用 `footer.share: true` 恢复全局服务。所有字段及关闭、继承方式见 [Front Matter 参考](/wiki/stellar/reference/front-matter/)。
