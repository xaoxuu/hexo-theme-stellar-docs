...
激活/悬停时菜单项及左栏列表项（最近更新、页面树、链接列表等）使用半透明背景高亮，并在顶部叠加轻微白色光照渐变与高光边，深色模式下透明度自动降低。集合的 list/grid/summary 条目静止时背景均透明，并在状态切换时直接更新背景、文字和图标，不使用过渡动画。非 glass 容器中的激活/悬停背景统一为 `var(--block)`。
