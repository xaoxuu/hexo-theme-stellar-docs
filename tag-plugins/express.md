...
> 主题图标默认异步加载：首屏关键图标（搜索、菜单、移动端侧栏按钮、返回）与 TOC 底部操作按钮（回到顶部、参与讨论）由模板内联，其余图标输出轻量占位符，页面按命名空间拉取 `js/icons/{ns}.json` 后原位替换为完整 SVG——最终效果与内联一致，只是图标数据不再随每个页面重复传输；站点在 `source/_data/icons.yml` 覆盖或新增的图标同样生效，无需额外配置。
